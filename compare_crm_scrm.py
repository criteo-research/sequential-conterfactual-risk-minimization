    ...
    for dataset in args.datasets:
        ...
        def _run(l, scrm):
            crm_losses = run_crm(X_train, y_train, X_test, y_test, pi0, samples,
                                 ...
                                 autotune_lambda=False, lambda_=l, scrm=scrm,
                                 truevar=False, ips_ix=args.ips_ix and scrm)
            return np.mean(crm_losses), np.std(crm_losses)

        all_losses_stds_crm = Parallel(n_jobs=args.n_jobs)(delayed(_run)(l, False) for l in args.lambda_grid)
        crm_best_loss_a_posteriori_mean, crm_best_loss_a_posteriori_std = sorted(all_losses_stds_crm)[0]
        print(' CRM best loss a posteriori: %.5f +/- %.5f' % (
            crm_best_loss_a_posteriori_mean, crm_best_loss_a_posteriori_std)
        )

        all_losses_stds_scrm = Parallel(n_jobs=args.n_jobs)(delayed(_run)(l, True) for l in args.lambda_grid)
        scrm_best_loss_a_posteriori_mean, scrm_best_loss_a_posteriori_std = sorted(all_losses_stds_scrm)[0]
        ...
        results['CRM'] += ['$%.5f \pm %.5f$' % (crm_best_loss_a_posteriori_mean, crm_best_loss_a_posteriori_std)]
